...
export const getStaticProps: GetStaticProps<{
  ...
  const res = await fetch(process.env.URL! + "/api/github", { method: "GET" });
  const pinnableItemConnection = await res.json();
...
const Collections = ({
  ...
  React.useEffect(() => {
    if (pinnableItemConnection) console.log("debug", pinnableItemConnection);
  }, [pinnableItemConnection]);
  return (
    <Wrapper>
      ...
      <div className="grid grid-cols-1 gap-16 sm:grid-cols-2">
        {pinnableItemConnection.user.pinnedItems.nodes.map((repo, index) => (
          <div key={index} className="group relative flex flex-col items-start">
            ...
            <p className="relatvie z-10 mt-2 text-sm text-zinc-600 dark:text0zinc-400">
              {repo.description}
            </p>
